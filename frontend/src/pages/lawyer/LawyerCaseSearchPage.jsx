...
/* ── Event type config (same palette as court) ───────────────── */
const EVENT_CONFIG = {
...
function TimelineEvent({ event }) {
  const cfg = EVENT_CONFIG[event.type] || {
    icon: <HiOutlineEye />, label: event.type, color: "#64748b", bg: "#64748b10",
  };
    ...
    <div style={{ display: "flex", gap: "1rem", marginBottom: "1.5rem" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "0.4rem" }}>
          <span style={{ fontWeight: 700, fontSize: "0.88rem", color: cfg.color }}>{cfg.label}</span>
          <span style={{ fontSize: "0.73rem", color: "var(--text-muted)", whiteSpace: "nowrap", marginLeft: 8 }}>{ts}</span>
        </div>
        <div style={{ fontSize: "0.84rem", lineHeight: 1.75 }}>
          {Object.entries(data)
            ...
            .map(([k, v]) => (
              <div key={k}>
                ...
                <span>{String(v).length > 220 ? String(v).slice(0, 220) + "…" : String(v)}</span>
              </div>
            ))}
        </div>
        {data.blockchain_tx && (
          <div style={{
            marginTop: "0.5rem", padding: "3px 8px", background: "#05966912",
            borderRadius: "6px", fontSize: "0.72rem", color: "#059669",
            fontFamily: "monospace", display: "inline-flex", alignItems: "center", gap: 5,
          }}>
            <HiOutlineShieldCheck /> {data.blockchain_tx.slice(0, 28)}…
          </div>
        )}
      </div>
    </div>
