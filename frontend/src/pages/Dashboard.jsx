  ...
  return (
    <section>
      <div className="page-header">
        ...
        <Link href="/fir/new" className="btn">+ Register FIR</Link>
      </div>
      ...
      <div className="stats-row">
        ...
        <StatCard href="/fir?status=under_investigation" icon={<HiOutlineArchive />} value={stats.under} label="Under Investigation" color="green" />
        <StatCard href="/chargesheet" icon={<HiOutlineClipboardCheck />} value={stats.chargesheeted} label="Chargesheeted" color="red" />
      </div>
      ...
      <div className="page-header" style={{ marginTop: "2rem" }}>
        ...
        <Link href="/fir" className="btn btn-secondary">View All</Link>
      </div>
      ...
      <div className="table-wrap">
        {recent.length > 0 && (
          <table className="table">
            <thead>
              <tr>
                <th>FIR Number</th><th>Police Station</th><th>Offence</th><th>Status</th><th>Registered</th><th></th>
              </tr>
            </thead>
            <tbody>
              {recent.map((f) => (
                <tr key={f.id}>
                  ...
                  <td style={{ maxWidth: 200, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{f.offence_sections}</td>
                  <td><span className={`badge badge-${f.status === "open" ? "blue" : f.status === "chargesheeted" ? "green" : "yellow"}`}>{f.status?.replace("_", " ").toUpperCase()}</span></td>
                  <td>{new Date(f.registered_at).toLocaleDateString("en-IN")}</td>
                  <td><Link href={`/fir/${f.id}`} className="btn btn-secondary" style={{ padding: "4px 12px", fontSize: "0.8rem" }}>View</Link></td>
                </tr>
              ))}
            </tbody>
          </table>
