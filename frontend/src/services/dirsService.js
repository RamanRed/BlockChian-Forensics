...
const seizureService = {
  ...
  downloadPropertyFile: async (propertyId) => {
    const response = await api.get(`/seizure/property/${propertyId}/download`, {
      responseType: 'blob', // Critical for file downloads
    });
    // Create a temporary URL to force download
    ...
    let filename = `evidence_${propertyId}`;
